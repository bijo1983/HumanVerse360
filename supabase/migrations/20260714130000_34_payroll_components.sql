...
CREATE TABLE IF NOT EXISTS payroll_components (
  ...
  formula                   text,           -- calculation_type='formula'
  statutory_function        text,           -- calculation_type='statutory' → registry key
  percentage_of             text,           -- calculation_type='percentage' → base component_code
  ...
  rounding_precision        smallint,       -- NULL = country currency decimals
  applicable_employee_types text[],         -- NULL = all
  applicable_nationality    text,           -- 'citizen' | 'expat' | NULL = all
...
-- ------------------------------------------------------------
-- Platform component seeds per country
-- v(country, code, name, type, calc, formula, stat_fn, taxable, social, eosb, ord, nat)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- Data migration: default structure per company + component
-- assignments from the legacy fixed salary columns
-- ------------------------------------------------------------
DO $$
DECLARE
  comp RECORD;
...
BEGIN
  FOR comp IN SELECT id, country_code FROM companies LOOP
    ...
    -- Only for countries with seeded components
    ...
    -- Legacy column → component assignments (BASIC always; allowances when non-zero)
